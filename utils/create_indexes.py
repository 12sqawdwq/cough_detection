...
def create_indexes(args):
    ...
    with h5py.File(waveforms_hdf5_path, 'r') as hr:
        with h5py.File(indexes_hdf5_path, 'w') as hw:
            ...
            classes_num = config.classes_num  # 确保config.py中有classes_num=527
            
            # 获取原始标签数据
            original_target = hr['target'][:]
            
            # 修复标签维度逻辑
            if original_target.ndim == 1 or original_target.dtype == int:
                # 如果是单维度整数数组（如[[0,5], [3], ...]），转换为one-hot
                target = np.zeros((audios_num, classes_num), dtype=bool)
                for i, indices in enumerate(original_target):
                    if isinstance(indices, np.ndarray) or isinstance(indices, list):
                        target[i, indices] = True
                    else:
                        target[i, indices] = True
            else:
                # 如果已经是二维数组，直接转换类型
                target = np.array(original_target, dtype=bool)
            
            hw.create_dataset('audio_name', data=hr['audio_name'][:])
            hw.create_dataset('target', data=target, dtype=bool)
    ...
    print(f'Write to {indexes_hdf5_path} (shape: {target.shape})')
