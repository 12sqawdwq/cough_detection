...
class AudioSetDataset(object):
    def __init__(self, hdf5_path, indexes, sample_rate=32000):
        ...
        self.audio_names = self.hdf5_file['audio_name'][:]
        self.targets = self.hdf5_file['target'][:]
        self.waveforms = self.hdf5_file['waveform']

        # 确保索引在有效范围内
        if max(self.indexes) >= len(self.audio_names):
...
class Base(object):
    def __init__(self, indexes_hdf5_path, batch_size, black_list_csv, random_seed):
        ...
        with h5py.File(indexes_hdf5_path, 'r') as hf:
            self.audio_names = [name.decode() for name in hf['audio_name'][:]]
            self.targets = hf['target'][:]
            self.waveforms = hf['waveform'][:]  # 直接加载波形数据
         # 检查数据维度
        if len(self.targets.shape) == 1:
            self.targets = self.targets.reshape(-1, 1)  # 确保是二维数组
        # 修改为（强制指定二分类）
        self.audios_num = self.targets.shape[0]
        self.classes_num = 2  # 手动指定类别数为2
...
class TrainSampler(Base):
    def __init__(self, indexes_hdf5_path, batch_size, black_list_csv=None,
        ...
        self.hd5f_path = indexes_hdf5_path
...
class BalancedTrainSampler(Base):
    def __init__(self, indexes_hdf5_path, batch_size, black_list_csv=None,
        ...
        logging.info(f'Samples per class: {self.samples_num_per_class}')
