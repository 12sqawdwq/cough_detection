#!/bin/bash
WORKSPACE=${1:-"./data/workspace"}   # 修改默认工作区目录为 data/workspace，你也可以根据实际情况修改

# evaluation indexes
python3 utils/create_indexes.py create_indexes \
    --waveforms_hdf5_path=$WORKSPACE"/hdf5s/waveforms/cough_notcough_train_train.h5" \
    --indexes_hdf5_path=$WORKSPACE"/hdf5s/indexes/cough_notcough_train.h5"
